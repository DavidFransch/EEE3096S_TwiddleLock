...
epsilom         = 0.01 #allowable change in voltage without changing direction
combinationDirection = [0, 1, 0]#[left, right, left]
combinationTime = [1,1,1]
    ...
    for i in range(len(arr)):
        ...
        for j in range (len(arr)):
            ...
            if((arr[j] is not None)):
                if(arr[maxa_index]<arr[j]):
                    maxa_index=j
        arr[i+maxa_index] = arr[i] #replace element at max index with first element
        arr[i] = arr[maxa_index] #replace first element with max element
